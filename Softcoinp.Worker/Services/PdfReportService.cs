    ...
    public class PdfReportService : IPdfReportService
    {
        ...
        private void ComposeHeader(IContainer container, MonthlyAnalytics analytics)
        {
            container.Row(row =>
            {
                ...
                row.RelativeItem().AlignRight().Column(column =>
                {
                    ...
                    column.Item().Text($"Generado (Auto): {DateTime.Now:yyyy-MM-dd HH:mm}").FontSize(8).FontColor(Color.FromHex("#9E9E9E"));
                });
            });
        }
        ...
        private void ComposeContent(IContainer container, MonthlyAnalytics analytics)
        {
            container.PaddingVertical(10).Column(column =>
            {
                ...
                column.Item().PaddingTop(20).Text("3. Top 10 Visitantes Recurrentes").FontSize(14).Bold().FontColor(Color.FromHex("#3F51B5"));
                column.Item().Table(table =>
                {
                    ...
                    table.Header(header =>
                    {
                        ...
                        header.Cell().Text("Identificación");
                        header.Cell().Text("Visitas");
                    });
                    ...
                    for (int i = 0; i < analytics.Top10Visitantes.Count; i++)
                    {
                        ...
                        table.Cell().Text(v.Nombre);
                        table.Cell().Text(v.Documento);
                        table.Cell().Text(v.Visitas.ToString());
                    }
                });

                column.Item().PaddingTop(20).Text("4. Distribución de Motivos de Bloqueo").FontSize(14).Bold().FontColor(Color.FromHex("#3F51B5"));
                column.Item().Row(row =>
                {
